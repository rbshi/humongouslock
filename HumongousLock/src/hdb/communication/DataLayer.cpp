...
#define NORD_WAREHOUSE_SIZE_RD (64)
#define NORD_CUSTOMER_SIZE_RD (64)
#define NORD_DISTRICT_WR (64)

#define NORD_ITEM_RD (128)
#define NORD_STOCK_RDWR (64)
#define NORD_ORDLINE_WR (128)

#define PAYM_WAREHOUSE_SIZE_RD (64)
#define PAYM_DISTRICT_SIZE_RDWR (64)
#define PAYM_CUSTOMER_SIZE_RDWR (64)
#define PAYM_HISTORY_SIZE_WR (128)
...
void DataLayer::newOrder(uint32_t warehouse, bool remoteItemPresent, std::set<uint32_t> *involvedLockServers) {
  ...
  readFromProcess(targetLockServerGlobalRank, NORD_WAREHOUSE_SIZE_RD + NORD_CUSTOMER_SIZE_RD); // Get warehouse and customer info
  writeToProcess(targetLockServerGlobalRank, NORD_DISTRICT_WR); // Put district info
  writeToProcess(targetLockServerGlobalRank, ORDER_SIZE+NEW_ORDER_SIZE); // Put order and new order

  if (!remoteItemPresent) {
    readFromProcess(targetLockServerGlobalRank, NORD_ITEM_RD * 10);
    readFromProcess(targetLockServerGlobalRank, NORD_STOCK_RDWR * 10);
    writeToProcess(targetLockServerGlobalRank, NORD_STOCK_RDWR * 10);
    writeToProcess(targetLockServerGlobalRank, NORD_ORDLINE_WR * 10);

  } else {
    uint32_t remoteElements = involvedLockServers->size();
    uint32_t localElements = (10 >= remoteElements) ? 10 - remoteElements :  0;
    readFromProcess(targetLockServerGlobalRank, NORD_ITEM_RD * localElements);
    readFromProcess(targetLockServerGlobalRank, NORD_STOCK_RDWR * localElements);
    writeToProcess(targetLockServerGlobalRank, NORD_STOCK_RDWR * localElements);
    writeToProcess(targetLockServerGlobalRank, NORD_ORDLINE_WR * localElements);
    for (auto it = involvedLockServers->begin(); it != involvedLockServers->end(); ++it) {
      ...
      if(lockServerId != targetLockServerGlobalRank) {
        readFromProcess(lockServerId, NORD_ITEM_RD);
        readFromProcess(lockServerId, NORD_STOCK_RDWR);
        writeToProcess(lockServerId, NORD_STOCK_RDWR);
        writeToProcess(lockServerId, NORD_ORDLINE_WR);
      }
    }
  }
}
...
void DataLayer::payment(uint32_t warehouse) {
  ...
  readFromProcess(targetLockServerGlobalRank, PAYM_WAREHOUSE_SIZE_RD);
  readFromProcess(targetLockServerGlobalRank, PAYM_DISTRICT_SIZE_RDWR);
  writeToProcess(targetLockServerGlobalRank, PAYM_DISTRICT_SIZE_RDWR);
  readFromProcess(targetLockServerGlobalRank, PAYM_CUSTOMER_SIZE_RDWR);
  writeToProcess(targetLockServerGlobalRank, PAYM_CUSTOMER_SIZE_RDWR);
  writeToProcess(targetLockServerGlobalRank, PAYM_HISTORY_SIZE_WR);
}
